...
void cprng_read(mpz_t x)
{
  ...
  if (opt_random_data == NULL) {
    ...
    for (count = 0; count < degree / 8; count++) {
      if ((!*opt_random_data)||(!opt_random_data[1])) {
	fatal("not enough random data");
      }
      buf[count] = get_hex_value(opt_random_data[0])*16 + opt_random_data[1];
      opt_random_data = &opt_random_data[2];
    }
  }
...
int main(int argc, char *argv[])
{
  ...
  while((i = getopt(argc, argv, flags)) != -1)
    ...
    case 'r': opt_random_data = optarg; break;
...
#endif
            " [-x] [-q] [-Q] [-D] [-v]\n"
            " [-r random_hexadecimal_data_to_use_as_source_entropy]\n",
            stderr);
      if (opt_showversion)
